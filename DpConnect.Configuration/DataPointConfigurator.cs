    ...
    public class DataPointConfigurator
    {
        ...
        {                        
            ...
            foreach (var dataPoint in xmlConfig.Element(DpXmlConfiguration.Tag_DataPointDefinition).Elements())
            {
                ...
                // Рeгистрируем точкки для процессоров
                //В Dp есть <Processor/> и может быть не один. Мы проходимся по каждому, чтобы зарегистрировать для него точку
                foreach (var dpProcessorConfig in dataPoint.Elements(DpXmlConfiguration.Tag_Processor))
                {
                    //Здесь мы получаем объект процессора по имени, законфигурированным в точке <Processor Name ="ИМЯ">
                    var processor = _processors.First(x => (x.Name == dpProcessorConfig.Attribute(DpXmlConfiguration.Tag_Description).Value));

                    //И берём все его свойства. Проходимся по каждому свойству
                    var processorProps = processor.GetType().GetProperties();                    
                    foreach (var procProp in processorProps)
                    {
                        //Если имя свойства совпадает с именем, указанном в конфиге <Processor TargetProperty = "Имя свойства">, то работаем с ней
                        if (procProp.Name == dpProcessorConfig.Attribute(DpXmlConfiguration.Tag_TargetProperty).Value)
                        {
                            //Если точка еще не создавалась. Т.к. разные процессоры могут использовать одни и те же точки.
                            //И если точка уже создана, то для следующего процессора мы не выполняем алгоритм по созданию точки,
                            //А просто присвоим готовый объект в свойство процессора
                            if (dp == null) 
                            {
                            ...
                            //Теперь, когда точка создана, зарегана в провайдере, можно её присвоить процессору
